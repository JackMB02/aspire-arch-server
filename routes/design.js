...
// Save design project form as draft
router.post('/design_project/save-draft', async (req, res) => {
  try {
    ...
    const result = await query(
      ...
      [
        ...
        'design_project',
        JSON.stringify(data),
        draft_name || `Design Project Draft - ${new Date().toLocaleDateString()}`
      ]
    );
    ...
    res.status(201).json({
      success: true,
      message: 'Design project form saved as draft!',
      draft_id: draft.id,
      saved_at: draft.created_at
    });
  } catch (error) {
    console.error('❌ Design project draft save error:', error);
    res.status(500).json({
      success: false,
