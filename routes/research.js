...
router.get("/admin/stats", async (req, res) => {
    try {
        const [
            ...
            query("SELECT COUNT(*) FROM research_articles"),
            query(
                "SELECT COUNT(*) FROM research_articles WHERE category = $1",
                ["sustainable"]
            ),
            query(
                "SELECT COUNT(*) FROM research_articles WHERE category = $1",
            ...
            query(
                "SELECT COUNT(*) FROM research_articles WHERE is_featured = true"
            ),
        ]);
...
// @desc    Get single research article for admin
// @route   GET /api/research/admin/articles/:id
// @access  Private
router.get("/admin/articles/:id", async (req, res) => {
    try {
        ...
        console.error("Get research article by ID error:", error);
        res.status(500).json({
            success: false,
            message: "Error fetching research article",
        });
    }
});
...
router.post("/articles", async (req, res) => {
    try {
        const {
            ...
            description, // Accept snake_case too
            content,
            category,
            authors,
            publication_date,
            publicationDate, // Accept camelCase from frontend
            main_image, // Frontend sends 'main_image' which maps to 'image_url'
            mainImage, // Accept camelCase from frontend
            image_url, // Accept snake_case too
            gallery_images,
        ...
        const descriptionValue = description || abstract;
        const imageUrl = image_url || main_image || mainImage;
        const galleryImagesArray = gallery_images || galleryImages;
        const publicationDateValue = publication_date || publicationDate;
       ...
       RETURNING *`,
            [
                title,
                descriptionValue || "", // Map abstract to description
                content || "",
        ...
        const newArticle = {
            ...
            publication_date: publicationDateValue || result.rows[0].created_at,
            publicationDate: publicationDateValue || result.rows[0].created_at,
...
// @route   PUT /api/research/admin/articles/:id
// @access  Private
router.put("/admin/articles/:id", async (req, res) => {
    try {
        ...
        const {
            title,
            abstract, // Frontend sends abstract
            content,
            category,
            authors,
            publication_date,
            main_image, // Frontend sends main_image
            gallery_images,
            display_order,
            is_featured,
            is_published,
        ...
        console.log("📝 Updating research article:", id);
       ...
       SET title = $1, description = $2, content = $3, category = $4, 
           image_url = $5, display_order = $6, is_featured = $7, is_published = $8, 
       ...
       RETURNING *`,
            [
                ...
                content || "",
                category || "sustainable",
                main_image || null, // Map main_image to image_url
                display_order || 0,
                is_featured || false,
                is_published || true,
                id,
        ...
        const updatedArticle = {
            ...
            category: result.rows[0].category,
            authors: authors || "ASPIRE Research Team",
            publication_date: result.rows[0].updated_at,
            main_image: result.rows[0].image_url,
        ...
        console.log("✅ Research article updated successfully:", id);
        ...
        res.status(500).json({
            success: false,
            message: "Error updating research article: " + error.message,
        });
    }
});

// @desc    Update research article (legacy endpoint)
...
router.post("/sustainable-practices", async (req, res) => {
    try {
        const { title, description, icon_name, display_order, is_active } =
            req.body;
...
router.get("/admin/all", async (req, res) => {
    try {
        const [
            ...
            query(
                "SELECT * FROM research_articles ORDER BY display_order, created_at DESC"
            ),
            query(
                "SELECT * FROM sustainable_practices ORDER BY display_order, created_at DESC"
            ),
            query(
                "SELECT * FROM climate_strategies ORDER BY display_order, created_at DESC"
            ),
            query(
                "SELECT * FROM social_studies ORDER BY display_order, created_at DESC"
            ),
            query(
                "SELECT * FROM research_stats ORDER BY category, display_order"
            ),
        ]);
